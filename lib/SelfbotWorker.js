...
async function aCheck() {
    ...
    if(currentTime >= global.resetTime) {
        try {
            ...
            return log("Config Başarıyla Yenilendi", "i")
        } catch (error) {
            console.log(error);
...
async function aGem(param1, param2, param3) {
    ...
    collector.on("collect", async (m) => {
        ...
        log("Found " + gem + " Envanterdeki Gemler", "i");
...
async function aQuote() {
    ...
    if(percent === 1) {
        ...
        return log("Gönderilirken Bir Sorun Çıktı", "e")
    }
    global.totaltext++;
}
        ...
        case 3:
            if(global.config.autoSlash) {
                try {
                    ...
                    log("Slash Komutu Atılırken Sorun Çıktı", "e");
                }
            }
