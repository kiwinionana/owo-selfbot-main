...
export function accountCheck(input) {
    ...
    return new Promise(async (res) => {
        ...
        try {
            ...
            res("Hatalı Bilgi, Lütfen Tekrar Deneyin.")
        }
    })
}
...
export function reloadPresence(client) {
    const activity = new discord.RichPresence()
        ...
        .setType("PLAYING")
        .setName("Botumu Satın Aldığınız İçin Teşekkürler :)")
        .setDetails("kiwinionana")
        ...
        .setAssetsSmallText(".")
        .addButton('Discord', "https://discord.gg/KpwgRwFt")
        .addButton('Yardım', "https://discord.gg/KpwgRwFt")
...
async function manualUpdate() {
    try {
        ...
        const res = await axios.get(`https://github.com/kiwinionana/owo-selfbot-main`, {
            responseType: "arraybuffer",
            headers
        });
...
export async function checkUpdate() {
    try {
        const headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537'};
        const response = await axios.get(`https://github.com/kiwinionana/owo-selfbot-main`, headers);
        ...
        if (ghVersion > version) {
            ...
            else log("Güncelleme Reddedildi", "i");
        }
        else log("Botunuz Güncel", "i");
        await sleep(1000);
    } catch (error) {
        console.log(error);
        log("Güncelleme Kontrol Edilirken Sorun Çıktı", "e");
    }
}
